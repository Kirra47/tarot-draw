// Classical Zhouyi judgement and line texts for the explicit reference layer.
// The text is kept separate from site-authored modern notes and is never used as a hidden prompt.
// Source structure: leechhui/zhouyi data.js; canonical per-hexagram links point to Wikisource.
...
const BY_NUMBER = new Map(Object.values(HEXAGRAMS).map((entry) => [entry.number, Object.freeze({
  ...entry,
  yaoCi: Object.freeze([...(entry.yaoCi || [])]),
})]));

export const ZHOUYI_TEXT_SOURCE = Object.freeze({
  name: '周易 · 六十四卦经文对照',
  dataSource: 'leechhui/zhouyi data.js',
  canonicalUrl: 'https://zh.wikisource.org/zh-hans/易',
});

export function hexagramText(number) {
  return BY_NUMBER.get(Number(number)) || null;
}

export function movingLineText(number, line) {
  const entry = hexagramText(number);
  const index = Number(line) - 1;
  return entry && Number.isInteger(index) && index >= 0 && index < entry.yaoCi.length
    ? entry.yaoCi[index]
    : '';
}

export const ZHOUYI_HEXAGRAM_COUNT = BY_NUMBER.size;
